...
public sealed class ScoreSystem : MonoBehaviour
{
    ...
    private int[] GetTeamRankingScores(Ranking rankingAsset)
    {
        ...
        PlayableTeam playableTeam =
            teamSetup.PlayableTeam;

        int[] rankingScores =
            playableTeam switch
            {
                PlayableTeam.TeamSonic =>
                    rankingAsset.teamSonic,

                PlayableTeam.TeamDark =>
                    rankingAsset.teamDark,

                PlayableTeam.TeamRose =>
                    rankingAsset.teamRose,

                PlayableTeam.TeamChaotix =>
                    rankingAsset.teamChaotix,

                _ => throw new System.ArgumentOutOfRangeException(
                    nameof(playableTeam),
                    playableTeam,
                    "Unsupported playable team.")
            };

        if (rankingScores == null)
        {
            Debug.LogError(
                $"No ranking table is configured for {playableTeam}.",
                this);
        }
